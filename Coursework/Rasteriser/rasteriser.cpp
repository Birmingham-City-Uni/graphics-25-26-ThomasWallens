	...
	const std::vector<uint8_t>& albedoTexture, int texWidth, int texHeight)
{
	...
	for (int x = minX; x <= maxX; ++x)
		for (int y = minY; y <= maxY; ++y) {
			...
			Eigen::Vector3f albedo = Eigen::Vector3f(texColor.r, texColor.g, texColor.b);
			albedo /= 255;
			// *** END YOUR CODE ***
...
int main()
{
	...
	Eigen::Matrix4f cameraToWorld = translationMatrix(Eigen::Vector3f(0.f, 0.2f, 0.f)) * rotateXMatrix(0.0f);
	...
	lights.emplace_back(new AmbientLight(Eigen::Vector3f(1.0f, 1.0f, 1.0f)));
	...
	Mesh bunnyMesh = loadMeshFile("../Model/GlyphidGrunt.obj");
	...
	lodepng::decode(bunnyTexture, bunnyTexWidth, bunnyTexHeight, "../Model/Grunt-Base.png");

	bunnyTransform = translationMatrix(Eigen::Vector3f(0.0f, -1.0f, 9.f)) * rotateYMatrix(0);
	...
	bunnyTransform = translationMatrix(Eigen::Vector3f(-1.5f, -1.0f, 6.f)) * rotateYMatrix(-0.5);
	drawMesh(imageBuffer, zBuffer, bunnyMesh, bunnyTexture, bunnyTexWidth, bunnyTexHeight, bunnyTransform, worldToClip, lights, width, height);
	bunnyTransform = translationMatrix(Eigen::Vector3f(-2.0f, -1.0f, 5.f)) * rotateYMatrix(-0.7);
	drawMesh(imageBuffer, zBuffer, bunnyMesh, bunnyTexture, bunnyTexWidth, bunnyTexHeight, bunnyTransform, worldToClip, lights, width, height);
	bunnyTransform = translationMatrix(Eigen::Vector3f(-1.0f, 0.0f, 11.f)) * rotateYMatrix(0.4);
	drawMesh(imageBuffer, zBuffer, bunnyMesh, bunnyTexture, bunnyTexWidth, bunnyTexHeight, bunnyTransform, worldToClip, lights, width, height);
